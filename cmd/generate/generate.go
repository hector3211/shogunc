	...
	return &Generator{
		Config: ShogunConfig{
			Sql: SqlConfig{
				...
				Output: "../../tmp/internal/generated.sql.go",
...
func (g *Generator) Execute(cwd string) error {
	...
	_, err := codegen.GenerateGoDb(filepath.Join(g.Config.Sql.Output, "db.go"), string(g.Config.Sql.Driver), g.Config.Sql.Package)
	return err
	return err
}
...
func (g *Generator) LoadSchema() error {
	...
	packageSlice := strings.Split(g.Config.Sql.Package, "/")
	packageName := packageSlice[len(packageSlice)-1]
	genContent.WriteString("//go:build ignore\n\n")
	genContent.WriteString(fmt.Sprintf("package %s\n\n", packageName))
...
func (g *Generator) parseSqlFile(file *os.File, fileName string) error {
	...
	fullContent.WriteString("//go:build ignore\n\n")

	packageSlice := strings.Split(g.Config.Sql.Package, "/")
	packageName := packageSlice[len(packageSlice)-1]
	fullContent.WriteString(fmt.Sprintf("package %s\n\n", packageName))
